...
class FullyConnectedGraph(nn.Module):
    def __init__(self, input_features, hidden_features, output_features):
        ...
        self.fully_connected_2 = nn.Linear(hidden_features+4, hidden_features)
        self.fully_connected_3 = nn.Linear(hidden_features, output_features)
        self.gcn = GCN(6, 2048, 4)

    def forward(self, x, graph):
        graph_state = self.gcn(graph)
    
        #add info about previous action into the state, to prevent the model from taking the same actions repeatedly

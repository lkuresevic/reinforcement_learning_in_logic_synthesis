...
def extract_graph(abc):
    num_nodes = abc.numNodes()
    edge_index = []
    features = torch.zeros((numNodes, 6))  # Assuming 6 types of nodes as per your DGL code.

    for node_idx in range(numNodes):
        aig_node = abc.aigNode(nodeIdx)
        node_type = aig_node.nodeType() # !Perhaps there are more specific node types?
        features[node_idx][node_type] = 1.0

        if aig_node.hasFanin0():
            fanin = aigNode.fanin0()
            edge_index.append([fanin, nodeIdx])
        if aig_node.hasFanin1():
            fanin = aig_node.fanin1()
            edge_index.append([fanin, node_idx])

    # Convert edge_index to a torch tensor with shape [2, num_edges].
